...
public class MyLoggerAspect {
	...
	public void beforeAdvice(JoinPoint joinPoint) { // JoinP連接點
	...
	//後置通知(不論是否發生異常都會執行)
	@After(value = "pt1()")
	public void afterAdvice(JoinPoint joinPoint) { // JoinP連接點
		String methodName = joinPoint.getSignature().getName(); // 方法名稱
		Object[] args = joinPoint.getArgs(); // 方法參數
		System.out.printf("後置通知 Log: %s, %s%n", methodName, Arrays.toString(args));
	}
	
	//返回通知(可以設定 returing 的方法回傳值，但若有發生例外則不會通知)
